...
def create_arg_parser():
    ...
    parser = argparse.ArgumentParser(description='This script allows you to find all .avi files in the inputDirectory and convert them to mp4 files using ffmpeg')
...
def list_files(dir):
    return glob.glob(dir + "/*.avi")

def transcode_file(inputFile):
    outputFile = inputFile[:-3] + "mp4"
    ff = ffmpy.FFmpeg(
        inputs={inputFile: '-analyzeduration 100M -probesize 50M -vcodec hevc'},
        outputs={outputFile: '-c:v libx264 -strict -2 -y'},
    )
    print(ff.cmd)  # Print the ffmpeg command for debugging
    ff.run()
