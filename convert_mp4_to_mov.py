...
def create_arg_parser():
    ...
    parser = argparse.ArgumentParser(description='Convert .mp4 files in the input directory to MOV using FFmpeg.')
...
def list_files(input_dir):
    """Lists all .mp4 files in the specified directory."""
    return glob.glob(os.path.join(input_dir, "*.mp4"))

def transcode_file(input_file):
    """Transcodes the input .mp4 file to MOV format."""
    output_file = os.path.splitext(input_file)[0] + ".mov"
    # The output options are set to use the libx264 video codec and the aac audio codec,
    # which are widely compatible with the .mov container.
    ffmpeg_command = ffmpy.FFmpeg(
        inputs={input_file: None},
        outputs={output_file: '-c:v libx264 -preset fast -c:a aac -strict -2 -y'}
    )
